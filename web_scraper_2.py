...
for page in range(1,2):
    try:
        url = 'https://fintechnews.am/news/page/{}/'.format(page)
        html = requests.get(url=url,headers=headers,timeout=10)
        soup = BeautifulSoup(html.text,'lxml')
        sj_lst = soup.select('.is-title.post-title a')
        for m in sj_lst:
            try:
                url2 = m['href']
                html2 = requests.get(url=url2,headers = headers,timeout=10)
                soup2 = BeautifulSoup(html2.text,'lxml')
                title = soup2.select('.is-title.post-title')[0].text.strip()
                content = soup2.select('.post-content.cf.entry-content.content-spacious')[0].text.strip().replace('\n','')
                text1 = 'title:\n'+title+'\n'+'content:\n'+content
                text = text+text1+'\n------------------------------\n'
                print(url2,'success')
            except:
